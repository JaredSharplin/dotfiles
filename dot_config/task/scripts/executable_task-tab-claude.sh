...
build_walkthrough_prompt() {
  ...
  prompt="${prompt}\n2. Generate a Mermaid sequence diagram that models the domain this PR works in."
  prompt="${prompt}\n   Use real domain objects as participants (e.g. 'Employee', 'Shift', 'Break', 'ClockInAlgorithmLogic') — not layers or method names."
  prompt="${prompt}\n   Messages between participants should describe the actual business operation with concrete values where they matter (e.g. 'finish break at 12:28pm', 'round finish to 12:30pm', 'place auto break at 12:28pm — blocks rounding')."
  prompt="${prompt}\n   Use alt/else blocks to show conditional or branching paths (e.g. success vs failure, before-fix vs after-fix behaviour)."
  prompt="${prompt}\n   The diagram should make the system legible to someone who has never seen the code — a reader should understand what objects exist, what they do to each other, and what this PR changes about that interaction."
  prompt="${prompt}\n   Render it using a bash heredoc — ALWAYS use this exact form:\n   cat <<'MERMAID_EOF' | mermaid-ascii\n   <your diagram here>\n   MERMAID_EOF\n   Then explain in 2-3 sentences what the diagram shows about the domain."
