...
build_walkthrough_prompt() {
  ...
  prompt="${prompt}\n   Use real domain objects as participants (e.g. 'Shift', 'ClockedBreak', 'AutoBreak') — not layers or method names."
  prompt="${prompt}\n   Messages should describe the actual business operation with concrete values where they matter (e.g. 'round finish 12:28→12:30pm', 'place auto break at 12:28pm — blocks rounding')."
  prompt="${prompt}\n   Use a single alt/else block to show the before-fix vs after-fix behaviour if relevant."
  ...
  prompt="${prompt}\n4. Sort the changed files into this reading order before starting: tests first, then models/data layer, then service/domain logic, then controllers/jobs/views, then config and migrations last. State the order you'll follow before beginning."
  prompt="${prompt}\n5. For each changed file, in that order:"
  ...
  if [[ "$is_self_review" == "true" ]]; then
    prompt="${prompt}\n8. When I confirm I understand, find the active task for this PR:"
