...
build_walkthrough_prompt() {
  ...
  prompt="${prompt}\n2. Generate a Mermaid diagram of the PR's structure (flowchart for features/bug fixes, sequence for API/job flows, class for model changes)."
  prompt="${prompt}\n   Node labels must be self-explanatory to someone who hasn't read the code — never use bare method names. Each label should describe WHAT the step does and WHY it matters, e.g. 'round_clocked_breaks! → rounds break to nearest 5 min' not just 'round_clocked_breaks!'. For bug fix before/after diagrams, include the consequence at each key step (e.g. 'auto break anchored at raw 12:28pm → blocks rounding')."
