...
build_walkthrough_prompt() {
  ...
  prompt="${prompt}\n2. Generate a Mermaid diagram that models the DOMAIN this PR works in — the actual objects, their state, and how they interact. The goal is to make the system legible before reading any code."
  prompt="${prompt}\n   Choose the diagram type based on what illuminates the domain best:"
  prompt="${prompt}\n   - Sequence diagram: when the PR changes how objects communicate over time (method calls, events, callbacks)"
  prompt="${prompt}\n   - Class diagram: when the PR changes object structure, relationships, or responsibilities"
  prompt="${prompt}\n   - Flowchart: when the PR changes a decision or data-transformation pipeline, showing object state at each stage"
  prompt="${prompt}\n   Show the real domain objects (e.g. Shift, Break, ClockInAlgorithmLogic) with their relevant attributes and state — not just method names in boxes."
  prompt="${prompt}\n   For bug fix PRs: show a before/after that makes the broken state and fixed state obvious from the object perspective (e.g. what attribute had the wrong value, what object was in the wrong state)."
  prompt="${prompt}\n   Render it using a bash heredoc — ALWAYS use this exact form so special characters are handled correctly:\n   cat <<'MERMAID_EOF' | mermaid-ascii\n   <your diagram here>\n   MERMAID_EOF\n   Then explain in 2-3 sentences what the diagram shows about the domain."
