...
- **Don't turn a null result into a cause.** Finding nothing means something is absent only when your tool reports that class of thing, and when you waited longer than the thing takes — background work outlasts a page interaction by minutes. Where either is unproven, report the null result with its limit ("the list showed none; I can't confirm it lists them at all") and leave the cause to the parent. Two null results are still one open question, not a mechanism.
