...
8. **Trace the path, don't simulate the execution** — In Phase 1, you may trace the call chain to identify which methods are involved and where debug puts belong. You may NOT reason about what values variables hold at runtime or what branches are taken. That is what the debug output will tell you. If you catch yourself writing about what code "returns" or "produces" for specific inputs without having run it, stop — you are simulating, not observing.
